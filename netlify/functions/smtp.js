...
exports.handler = async (event, context) => {
  ...
  if (action === 'send-email') {
    ...
    try {
      ...
      // 注意: Netlify Functions では環境変数を直接更新することはできません
      // ここでは、成功メッセージを返すだけにします

      return { statusCode: 200, body: JSON.stringify({ message: 'SMTP settings updated and verified successfully' }) };
    } catch (error) {
      console.error('Error updating SMTP settings:', error);
      return { statusCode: 500, body: JSON.stringify({ message: 'Failed to update SMTP settings', error: error.message }) };
    }
  } else {
